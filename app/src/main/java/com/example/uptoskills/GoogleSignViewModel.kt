...
class GoogleSignInViewModel<GetGoogleIdOption : Any> : ViewModel() {
    ...
    fun handleGoogleSignIn(context: Context, navController: NavController) {
        viewModelScope.launch {
            ...
            this@GoogleSignInViewModel.googleSignIn(context).also { result ->
                result.run {
                    fold(onSuccess = { authResult ->
                        // Handle successful sign-in
                        val currentUser = authResult.user
                        if (currentUser != null) {
                            User(
                                currentUser.uid,
                                currentUser.displayName ?: "", // Handle nullable properties safely
                                currentUser.photoUrl?.toString()
                                    ?: "", // Handle nullable properties safely
                                currentUser.email ?: "" // Handle nullable properties safely
                            ).also { user.value = it }
                        ...
                        // Handle failure case
                        Toast.makeText(
                            context, "Sign-in failed: ${error.message}", Toast.LENGTH_LONG
                        ).show()
                    })
                }
            }
    ...
    private fun fold(onSuccess: Any, onFailure: Any) {
        TODO("Not yet implemented")
    }

    private fun googleSignIn(context: Context) = Unit
                            ...
                            try {
                                ...
                                val googleIdOption: GetGoogleIdOption =
                                    GetGoogleIdOption.Builder().setFilterByAuthorizedAccounts(false)
                                        ...
                                        .setNonce(hashedNonce).setAutoSelectEnabled(true).build()

                                        //                            val googleIdOption: GetGoogleIdOption =
                                        //                                GetGoogleIdOption.androidx.compose.foundation.layout.Box {
                                        //                                    Builder()
                                        //                                }
                                        ...
                                        // Create a credential request with the GoG)
                                        .build()
...
private fun Any.getCredential(context: Context, request: Nothing?): Any = Unit
